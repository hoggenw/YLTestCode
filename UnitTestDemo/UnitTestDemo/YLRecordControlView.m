...
-(void)initialUI {
    self.progressView = [[YLProgressView alloc] initWithFrame: CGRectMake(0, 0, self.frame.size.width, 20)];
...
-(void)stopOrChioceButtonAction:(UIButton *)sender {
    ...
    if (self.delegate != nil) {
        if (!sender.selected) {
            [self.delegate stopRecordDelegate];
        }else{
            [self.delegate choiceVideoDelegate];
        }
    }
...
-(void)cancalButtonAction {
     [self.progressView stopProgress];
    if (self.delegate != nil) {
        [self.delegate cancelRecordDelegate];
    }
}
