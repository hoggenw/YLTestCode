...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath {
    YLDatabaseQueue *q = [[self alloc] initWithPath:aPath];
...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath flags:(int)openFlags {
    YLDatabaseQueue *q = [[self alloc] initWithPath:aPath flags:openFlags];
...
- (void)inDatabase:(void (^)(YLDatabase *db))block {
#ifndef NDEBUG
    /* Get the currently executing queue (which should probably be nil, but in theory could be another DB queue
     * and then check it against self to make sure we're not about to deadlock. */
    YLDatabaseQueue *currentSyncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(YLDatabase *db, BOOL *rollback))block {
    FMDBRetain(self);
    dispatch_sync(_queue, ^() {
...
- (void)inTransaction:(void (^)(YLDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inDeferredTransaction:(void (^)(YLDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionDeferred withBlock:block];
}

- (void)inExclusiveTransaction:(void (^)(YLDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inImmediateTransaction:(void (^)(YLDatabase * _Nonnull, BOOL * _Nonnull))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(void (^)(YLDatabase *db, BOOL *rollback))block {
