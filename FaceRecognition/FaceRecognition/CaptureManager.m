...
- (UIImage *) faceImageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer
{
    //获取灰度图像数据
...
//    YLFaceDirectionType faceOrientation = [self faceImageOrientation];
//
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(onOutputFaceImage:)]){
        UIImage* faceImage=[self faceImageFromSampleBuffer:sampleBuffer];
...
-(YLFaceDirectionType)faceImageOrientation
{
    YLFaceDirectionType faceOrientation = YLFaceDirectionTypeLeft;
    BOOL isFrontCamera=self.videoDeviceInput.device.position==AVCaptureDevicePositionFront;
    switch (self.interfaceOrientation) {
        case UIDeviceOrientationPortrait:{//
            faceOrientation = YLFaceDirectionTypeLeft;
        }
            break;
        case UIDeviceOrientationPortraitUpsideDown:{
            faceOrientation = YLFaceDirectionTypeRight;
        }
            break;
        case UIDeviceOrientationLandscapeRight:{
            faceOrientation = isFrontCamera ? YLFaceDirectionTypeUp : YLFaceDirectionTypeDown;
        }
            break;
        default:{//
            faceOrientation = isFrontCamera ? YLFaceDirectionTypeDown : YLFaceDirectionTypeUp;
        }
            break;
    }
